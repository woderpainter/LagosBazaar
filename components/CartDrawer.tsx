    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex">
        <div className="w-screen max-w-md">
          <div className="h-full flex flex-col bg-surface shadow-xl">
            <div className="flex-1 py-6 overflow-y-auto px-4 sm:px-6">
              <div className="flex items-start justify-between">
                <h2 className="text-lg font-medium text-text">Shopping Cart</h2>
                <button
                  type="button"
                  className="-m-2 p-2 text-text-subdued hover:text-text"
                  onClick={onClose}
                >
                ...
                <div className="flow-root">
                  {cartItems.length === 0 ? (
                    <div className="text-center py-10">
                        ...
                        <p className="mt-2 text-text-subdued">Your cart is empty.</p>
                    ...
                    <ul role="list" className="-my-6 divide-y divide-gray-200">
                        ...
                        <li key={product.id} className="py-6 flex">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm text-text-subdued">{product.category}</p>
                            </div>
                            <div className="flex-1 flex items-end justify-between text-sm">
                                <div className="flex items-center border border-border rounded-md">
                                    <button 
                                        onClick={() => onUpdateQuantity(product.id, -1)}
                                        className="px-2 py-1 text-text hover:bg-gray-100 disabled:opacity-50"
                                        disabled={product.quantity <= 1}
                                    >-</button>
                                    <span className="px-2 font-medium">{product.quantity}</span>
                                    <button 
                                        onClick={() => onUpdateQuantity(product.id, 1)}
                                        className="px-2 py-1 text-text hover:bg-gray-100"
                                    >+</button>
                                </div>
            ...
            {cartItems.length > 0 && (
                ...
                <p className="mt-0.5 text-sm text-text-subdued">Shipping and taxes calculated at checkout.</p>
                <div className="mt-6">
                    <button
                        onClick={onCheckout}
                        className="w-full flex justify-center items-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-surface bg-primary hover:bg-primary-dark"
                    >
                        Checkout <ArrowRight className="ml-2 h-5 w-5" />
                    </button>
                </div>
                <div className="mt-6 flex justify-center text-sm text-center text-text-subdued">
                    <p>
                    or{' '}
                    <button
                        type="button"
